...
def get_section(url):
	...
	while True:
		try:
			...
			idx = payload.index('\t\tdocId')
			outfilename = payload[idx:idx+30].split('"')[1]
			print(outfilename)
...
def fetch_year_range(y0,y1):
	for year in range(y0,y1):
		for infile in sorted(glob.glob('data/filtered/%s*' % year), reverse=True):
			...
			outfile  = 'data/codes_for_only_News/%s.txt' % filename

			if os.path.isfile(outfile):
				print('%s already exists' % outfile)
			else:
				print('Running %s.csv' % filename)
				df = pd.read_csv(infile, names=['date','url_article','page', 'type'])
				df = df[df['type']=='News']

				#df['outfile'] = df.url_article.apply(get_section)
				#df.to_csv(outfile, index=False, header=False )

				outfilenames = [ get_section(x) for x in df.url_article ]
